...
export const projects: Project[] = [
...

  {
    ...
    year: '2026',
    live: null,
    github: 'https://github.com/chris-bhaila/integrity-check',
    tagline: 'Dual-signal plagiarism detection combining from-scratch lexical shingling with SBERT semantic similarity — catches paraphrasing, not just copy-paste.',
    problem: 'Traditional plagiarism checkers rely on exact phrase matching, so a submission that\'s been reworded — same ideas, different words — sails through undetected, even though the copying is just as real.',
    role: 'Solo academic project (final-semester) — I implemented both similarity algorithms from scratch, built the FastAPI scoring service, and designed the full Laravel platform including the role-based review workflow.',
    stat: { value: '2', label: 'independent similarity signals combined into one score' },
    pullQuote: 'Reworded submissions get caught even when not a single phrase matches.',
    overview: 'Standard plagiarism detection catches copy-paste but misses paraphrasing entirely — if the wording changes, exact-match methods find nothing. IntegrityCheck combines two independently-built signals: n-gram shingling with Jaccard similarity for verbatim overlap, and Sentence-BERT embeddings with cosine similarity for semantic meaning. A submission that shares zero exact phrases with a source can still be flagged if the underlying ideas match — which is exactly the case exact-match tools miss.',
    approach: 'Both similarity algorithms were implemented from scratch rather than pulled from a library — the shingling/Jaccard lexical scorer and the cosine similarity function both hand-written, with only the SBERT embedding model itself pretrained. The two scores are combined into a single weighted result per comparison, tuned against real test cases (verbatim, paraphrased, and unrelated text) to confirm each signal was catching what the other missed. The system checks submissions against both other student work and the open web, using the Google Custom Search API to find candidate sources and a scraping pipeline to extract comparison text — routed through the same scoring pipeline either way. A Laravel frontend handles the role-based workflow (student, teacher, admin) with a queued job pipeline so similarity checks run asynchronously against the Python FastAPI scoring service.',
    impact: [
      'Dual-signal design validated against verbatim, paraphrased, and unrelated test cases',
      'Semantic similarity catches reworded plagiarism with zero lexical overlap',
      'Extended beyond peer comparison to open-web source checking',
      'Full role-based review workflow with highlighted passage-level overlap',
    ],
    impactStats: [
      { value: '2', label: 'similarity signals — lexical (from scratch) and semantic (SBERT + custom cosine)' },
      { value: '3', label: 'user roles — student, teacher, admin' },
      { icon: 'search', phrase: 'Checks against both peer submissions and the open web' },
    ],
    architecture: {
      flow: ['Student Submission', 'Laravel', 'Queued Job', 'FastAPI Scoring Service', 'Similarity Report'],
      branchFrom: 'FastAPI Scoring Service',
